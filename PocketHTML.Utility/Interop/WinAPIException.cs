...
namespace OpenNETCF.Win32
{
	...
	public class WinAPIException : Exception
	{
		...
		{
			IntPtr pBuffer;
			int nLen = CoreDLL.FormatMessage(CoreDLL.FormatMessageFlags.FromSystem | CoreDLL.FormatMessageFlags.AllocateBuffer, 0, ErrNo, 0, out pBuffer, 0, null);
			if ( nLen == 0 )//Failed
			{
				return string.Format("Error {0} (0x{0:X})", ErrNo);
			}
			string sMsg = Marshal.PtrToStringUni(pBuffer, nLen);
			CoreDLL.LocalFreeCE(pBuffer);
			return sMsg;
		}
	}
}
